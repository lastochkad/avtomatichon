...
@allure.description("Заполнение формы валидными данными")
@allure.feature("Test-2")
@allure.step("Заполняем форму и проверяем поля")
def test_fill_form_and_verify_highlight(driver_setup):
    ...
    with allure.step("Проверка цвета поля зип-кода полученого результата с ожидаемым"):
